<div id="widgetkit" class="wrap">
	...
	<form id="form" method="post" action="<?php echo $this['system']->link(array('task' => "save_{$type}_joomla")); ?>">
        ...
        <div class="form">
            ...
            <div class="zoo box">
				<h3>Joomla Articles</h3>
				<div class="content">
					<?php

						$settings = array();

						if ($setting = $joomla_xml->xpath('params/param')) {
							$settings = array_merge($settings, $setting);
						}
                        ...
                        foreach ($settings as $setting) {
                            ...
                            $name    = (string) $setting->attributes()->name;
                            $default = (string) $setting->attributes()->default;
                            $value   = isset($widget->joomla[$name]) ? $widget->joomla[$name] : $default;

                            echo '<div class="option">';
                            echo '<h4>'.$label.'</h4>';
                            echo '<div class="value">';
                            echo $this['field']->render($type, 'joomla['.$name.']', $value, $setting);
                            echo '</div>';
                            echo '</div>';
