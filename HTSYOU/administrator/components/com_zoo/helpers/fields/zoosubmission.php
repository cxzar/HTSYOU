...
foreach ($table->all(array('order' => 'name')) as $application) {
	...
	foreach($application->getSubmissions() as $submission) {
		...
		if ($show_types) {
			...
			$types[] = $this->app->html->_('select.genericlist', $type_options, $control_name.'[type]', $attribs, 'value', 'text', $params->get('type'));
		}
	}
	...
	$submissions[] = $this->app->html->_('select.genericlist', $submission_options, $control_name.'[submission]', $attribs, 'value', 'text', $params->get('submission'));
}
...
$html[] = $this->app->html->_('select.genericlist', $app_options, $control_name.'['.$name.']', 'class="application"', 'value', 'text', $value);
