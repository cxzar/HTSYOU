...
class ItemTable extends AppTable {
	...
	*/
	...
	/*
		@deprecated ZOO version 2.5 beta
	*/
	public function getFromCategory($application_id, $category_id, $published = false, $user = null, $orderby = "", $offset = 0, $limit = 0) {
		return $this->getFromCategory($application_id, $category_id, $published, $user, $orderby, $offset, $limit);
	}
	...
	protected function _getItemOrder($order) {
		...
		foreach ($order as $element) {
			if (strpos($element, '_item') === 0) {
				...
				$result['query'] = "a.$var $reversed";
				return $result;
			}
		}

		// order by elements
		$result['join'] = " LEFT JOIN ".ZOO_TABLE_SEARCH." AS s ON a.id = s.item_id AND s.element_id IN ('".implode("', '", $order)."')";
		$result['query'] = $reversed == 'ASC' ? "ISNULL(s.value), s.value" : "s.value DESC";
