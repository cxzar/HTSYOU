		...
		try {
			...
			foreach ($items as $item) {
				try {
					...
					$this->app->error->raiseNotice(0, sprintf("Error updating search data for item with id %s. (%s)", $item->id, $e));
				}
			}

			$msg = JText::_(sprintf('Cleaned database (Removed %s entries) and items search data has been updated.', $row));

		} catch (Exception $e) {

			$msg = '';
			$this->app->error->raiseNotice(0, "Error cleaning database. ($e)");
