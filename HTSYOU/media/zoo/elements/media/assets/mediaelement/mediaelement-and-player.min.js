...
!(typeof this.nav.mimeTypes!="undefined"&&this.nav.mimeTypes[a]&&!this.nav.mimeTypes[a].enabledPlugin)){e=d.replace(b,"").replace(/^\s+/,"").replace(/\sr/gi,".").split(".");for(b=0;b<e.length;b++)e[b]=parseInt(e[b].match(/\d+/),10)}}else if(typeof window.ActiveXObject!="undefined")try{if(f=new ActiveXObject(d))e=c(f)}catch(g){}return e}};
...
mejs.PluginMediaElement.prototype={pluginElement:null,pluginType:"",isFullScreen:false,playbackRate:-1,defaultPlaybackRate:-1,seekable:[],played:[],paused:true,ended:false,seeking:false,duration:0,error:null,muted:false,volume:1,currentTime:0,play:function(){if(this.pluginApi!=null)this.pluginApi.playMedia(),this.paused=false},load:function(){if(this.pluginApi!=null)this.pluginApi.loadMedia(),this.paused=false},pause:function(){if(this.pluginApi!=null)this.pluginApi.pauseMedia(),this.paused=true},
stop:function(){if(this.pluginApi!=null)this.pluginApi.stopMedia(),this.paused=true},canPlayType:function(b){var a,d,c,e=mejs.plugins[this.pluginType];for(a=0;a<e.length;a++)if(c=e[a],mejs.PluginDetector.hasPluginVersion(this.pluginType,c.version))for(d=0;d<c.types.length;d++)if(b==c.types[d])return true;return false},setSrc:function(b){if(typeof b=="string")this.pluginApi.setSrc(mejs.Utility.absolutizeUrl(b)),this.src=mejs.Utility.absolutizeUrl(b);else{var a,d;for(a=0;a<b.length;a++)if(d=b[a],this.canPlayType(d.type))this.pluginApi.setSrc(mejs.Utility.absolutizeUrl(d.src)),
...
this.pluginApi.setVideoSize(b,a)},setFullscreen:function(b){this.pluginApi!=null&&this.pluginApi.setFullscreen(b)},enterFullScreen:function(){this.setFullscreen(true)},enterFullScreen:function(){this.setFullscreen(false)},addEventListener:function(b,a){this.events[b]=this.events[b]||[];this.events[b].push(a)},removeEventListener:function(b,a){if(!b)return this.events={},true;var d=this.events[b];if(!d)return true;if(!a)return this.events[b]=[],true;for(i=0;i<d.length;i++)if(d[i]===a)return this.events[b].splice(i,
1),true;return false},dispatchEvent:function(b){var a,d,c=this.events[b];if(c){d=Array.prototype.slice.call(arguments,1);for(a=0;a<c.length;a++)c[a].apply(null,d)}}};
...
(function(b){mejs.MepDefaults={poster:"",defaultVideoWidth:480,defaultVideoHeight:270,videoWidth:-1,videoHeight:-1,audioWidth:400,audioHeight:30,startVolume:0.8,loop:false,enableAutosize:true,alwaysShowHours:false,showTimecodeFrameCount:false,framesPerSecond:25,autosizeProgress:true,alwaysShowControls:false,iPadUseNativeControls:false,iPhoneUseNativeControls:false,AndroidUseNativeControls:false,features:"playpause,current,progress,duration,tracks,volume,fullscreen".split(","),isVideo:true};mejs.mepIndex=
0;mejs.MediaElementPlayer=function(a,d){if(!(this instanceof mejs.MediaElementPlayer))return new mejs.MediaElementPlayer(a,d);this.$media=this.$node=b(a);this.node=this.media=this.$media[0];if(typeof this.node.player!="undefined")return this.node.player;else this.node.player=this;this.options=b.extend({},mejs.MepDefaults,d);this.init();return this};mejs.MediaElementPlayer.prototype={init:function(){var a=this,d=mejs.MediaFeatures,c=b.extend(true,{},a.options,{success:function(b,c){a.meReady(b,c)},
error:function(b){a.handleError(b)}}),e=a.media.tagName.toLowerCase();a.isDynamic=e!=="audio"&&e!=="video";a.isVideo=a.isDynamic?a.options.isVideo:e!=="audio"&&a.options.isVideo;if(d.isiPad&&a.options.iPadUseNativeControls||d.isiPhone&&a.options.iPhoneUseNativeControls)a.$media.attr("controls","controls"),a.$media.removeAttr("poster"),d.isiPad&&a.media.getAttribute("autoplay")!==null&&(a.media.load(),a.media.play());else if(!d.isAndroid||!a.AndroidUseNativeControls)a.$media.removeAttr("controls"),
a.id="mep_"+mejs.mepIndex++,a.container=b('<div id="'+a.id+'" class="mejs-container"><div class="mejs-inner"><div class="mejs-mediaelement"></div><div class="mejs-layers"></div><div class="mejs-controls"></div><div class="mejs-clear"></div></div></div>').addClass(a.$media[0].className).insertBefore(a.$media),a.container.addClass((d.isAndroid?"mejs-android ":"")+(d.isiOS?"mejs-ios ":"")+(d.isiPad?"mejs-ipad ":"")+(d.isiPhone?"mejs-iphone ":"")+(a.isVideo?"mejs-video ":"mejs-audio ")),d.isiOS?(d=a.$media.clone(),
a.container.find(".mejs-mediaelement").append(d),a.$media.remove(),a.$node=a.$media=d,a.node=a.media=d[0]):a.container.find(".mejs-mediaelement").append(a.$media),a.controls=a.container.find(".mejs-controls"),a.layers=a.container.find(".mejs-layers"),a.isVideo?(a.width=a.options.videoWidth>0||a.options.videoWidth.toString().indexOf("%")>-1?a.options.videoWidth:a.media.style.width!==""&&a.media.style.width!==null?a.media.style.width:a.media.getAttribute("width")!==null?a.$media.attr("width"):a.options.defaultVideoWidth,
a.height=a.options.videoHeight>0||a.options.videoHeight.toString().indexOf("%")>-1?a.options.videoHeight:a.media.style.height!==""&&a.media.style.height!==null?a.media.style.height:a.$media[0].getAttribute("height")!==null?a.$media.attr("height"):a.options.defaultVideoHeight):(a.width=a.options.audioWidth,a.height=a.options.audioHeight),a.setPlayerSize(a.width,a.height),c.pluginWidth=a.height,c.pluginHeight=a.width;mejs.MediaElement(a.$media[0],c)},controlsAreVisible:true,showControls:function(a){var b=
this;if(!b.controlsAreVisible)typeof a=="undefined"||a?(b.controls.css("visibility","visible").stop(true,true).fadeIn(200,function(){b.controlsAreVisible=true}),b.container.find(".mejs-control").css("visibility","visible").stop(true,true).fadeIn(200,function(){b.controlsAreVisible=true})):(b.controls.css("visibility","visible").css("display","block"),b.container.find(".mejs-control").css("visibility","visible").css("display","block"),b.controlsAreVisible=true),b.setControlsSize()},hideControls:function(a){var d=
this;if(d.controlsAreVisible)typeof a=="undefined"||a?(d.controls.stop(true,true).fadeOut(200,function(){b(this).css("visibility","hidden").css("display","block");d.controlsAreVisible=false}),d.container.find(".mejs-control").stop(true,true).fadeOut(200,function(){b(this).css("visibility","hidden").css("display","block")})):(d.controls.css("visibility","hidden").css("display","block"),d.container.find(".mejs-control").css("visibility","hidden").css("display","block"),d.controlsAreVisible=false)},
...
j=d.find(".mejs-time-handle"),h=d.find(".mejs-time-float"),l=d.find(".mejs-time-float-current"),k=function(a){var a=a.pageX,b=f.offset(),c=f.outerWidth(),d=0;a>b.left&&a<=c+b.left&&e.duration&&(d=(a-b.left)/c,d=d<=0.02?0:d*e.duration,m&&e.setCurrentTime(d),h.css("left",a-b.left),l.html(mejs.Utility.secondsToTimeCode(d)))},m=false,n=false;f.bind("mousedown",function(a){if(a.which===1)return m=true,k(a),false});d.find(".mejs-time-total").bind("mouseenter",function(){n=true}).bind("mouseleave",function(){n=
false});b(document).bind("mouseup",function(){m=false}).bind("mousemove",function(a){(m||n)&&k(a)});e.addEventListener("progress",function(b){a.setProgressRail(b);a.setCurrentRail(b)},false);e.addEventListener("timeupdate",function(b){a.setProgressRail(b);a.setCurrentRail(b)},false);this.loaded=c;this.total=f;this.current=g;this.handle=j},setProgressRail:function(a){var b=a!=void 0?a.target:this.media,c=null;b&&b.buffered&&b.buffered.length>0&&b.buffered.end&&b.duration?c=b.buffered.end(0)/b.duration:
b&&b.bytesTotal!=void 0&&b.bytesTotal>0&&b.bufferedBytes!=void 0?c=b.bufferedBytes/b.bytesTotal:a&&a.lengthComputable&&a.total!=0&&(c=a.loaded/a.total);c!==null&&(c=Math.min(1,Math.max(0,c)),this.loaded&&this.total&&this.loaded.width(this.total.width()*c))},setCurrentRail:function(){if(this.media.currentTime!=void 0&&this.media.duration&&this.total&&this.handle){var a=this.total.width()*this.media.currentTime/this.media.duration,b=a-this.handle.outerWidth(true)/2;this.current.width(a);this.handle.css("left",
b)}}})})(mejs.$);
(function(b){b.extend(mejs.MepDefaults,{duration:-1});b.extend(MediaElementPlayer.prototype,{buildcurrent:function(a,d,c,e){b('<div class="mejs-time"><span class="mejs-currenttime">'+(a.options.alwaysShowHours?"00:":"")+(a.options.showTimecodeFrameCount?"00:00:00":"00:00")+"</span></div>").appendTo(d);this.currenttime=this.controls.find(".mejs-currenttime");e.addEventListener("timeupdate",function(){a.updateCurrent()},false)},buildduration:function(a,d,c,e){d.children().last().find(".mejs-currenttime").length>0?
b(' <span> | </span> <span class="mejs-duration">'+(this.options.duration>0?mejs.Utility.secondsToTimeCode(this.options.duration,this.options.alwaysShowHours||this.media.duration>3600,this.options.showTimecodeFrameCount,this.options.framesPerSecond||25):(a.options.alwaysShowHours?"00:":"")+(a.options.showTimecodeFrameCount?"00:00:00":"00:00"))+"</span>").appendTo(d.find(".mejs-time")):(d.find(".mejs-currenttime").parent().addClass("mejs-currenttime-container"),b('<div class="mejs-time mejs-duration-container"><span class="mejs-duration">'+
(this.options.duration>0?mejs.Utility.secondsToTimeCode(this.options.duration,this.options.alwaysShowHours||this.media.duration>3600,this.options.showTimecodeFrameCount,this.options.framesPerSecond||25):(a.options.alwaysShowHours?"00:":"")+(a.options.showTimecodeFrameCount?"00:00:00":"00:00"))+"</span></div>").appendTo(d));this.durationD=this.controls.find(".mejs-duration");e.addEventListener("timeupdate",function(){a.updateDuration()},false)},updateCurrent:function(){this.currenttime&&this.currenttime.html(mejs.Utility.secondsToTimeCode(this.media.currentTime,
...
h=f.find(".mejs-volume-current"),l=f.find(".mejs-volume-handle"),k=function(a){if(g.is(":visible")){var b=j.height(),c=j.position(),a=b-b*a;l.css("top",c.top+a-l.height()/2);h.height(b-a);h.css("top",c.top+a)}else g.show(),k(a),g.hide()},m=function(a){var b=j.height(),c=j.offset(),d=parseInt(j.css("top").replace(/px/,""),10),a=a.pageY-c.top;c.top!=0&&(c=Math.max(0,(b-a)/b),c=Math.min(c,1),a<0?a=0:a>b&&(a=b),l.css("top",a-l.height()/2+d),h.height(b-a),h.css("top",a+d),c==0?(e.setMuted(true),f.removeClass("mejs-mute").addClass("mejs-unmute")):
(e.setMuted(false),f.removeClass("mejs-unmute").addClass("mejs-mute")),c=Math.max(0,c),c=Math.min(c,1),e.setVolume(c))},n=false;f.hover(function(){g.show()},function(){g.hide()});g.bind("mousedown",function(a){m(a);n=true;return false});b(document).bind("mouseup",function(){n=false}).bind("mousemove",function(a){n&&m(a)});f.find("button").click(function(){e.setMuted(!e.muted)});e.addEventListener("volumechange",function(a){n||(e.muted?(k(0),f.removeClass("mejs-mute").addClass("mejs-unmute")):(k(a.target.volume),
f.removeClass("mejs-unmute").addClass("mejs-mute")))},false);k(a.options.startVolume);e.pluginType==="native"&&e.setVolume(a.options.startVolume)}})})(mejs.$);
(function(b){b.extend(mejs.MepDefaults,{forcePluginFullScreen:false,newWindowCallback:function(){return""},fullscreenText:"Fullscreen"});b.extend(MediaElementPlayer.prototype,{isFullScreen:false,isNativeFullScreen:false,docStyleOverflow:null,isInIframe:false,buildfullscreen:function(a,d){if(a.isVideo){a.isInIframe=window.location!=window.parent.location;mejs.MediaFeatures.hasTrueNativeFullScreen&&(console.log("added change event: "+mejs.MediaFeatures.fullScreenEventName),a.container.bind(mejs.MediaFeatures.fullScreenEventName,
function(){console.log("fullscreenchange event: "+mejs.MediaFeatures.isFullScreen());mejs.MediaFeatures.isFullScreen()?(a.isNativeFullScreen=true,a.setControlsSize()):(a.isNativeFullScreen=false,a.exitFullScreen())}));var c=this,e=b('<div class="mejs-button mejs-fullscreen-button"><button type="button" aria-controls="'+c.id+'" title="'+c.options.fullscreenText+'"></button></div>').appendTo(d).click(function(){mejs.MediaFeatures.hasTrueNativeFullScreen&&mejs.MediaFeatures.isFullScreen()||a.isFullScreen?
...
function(){a.container.find(".mejs-captions-position").addClass("mejs-captions-position-hover")}).bind("mouseleave",function(){e.paused||a.container.find(".mejs-captions-position").removeClass("mejs-captions-position-hover")});a.trackToLoad=-1;a.selectedTrack=null;a.isLoadingTrack=false;if(a.tracks.length>0&&a.options.translations.length>0)for(f=0;f<a.options.translations.length;f++)a.tracks.push({srclang:a.options.translations[f].toLowerCase(),src:null,kind:"subtitles",entries:[],isLoaded:false,
isTranslation:true});for(f=0;f<a.tracks.length;f++)a.tracks[f].kind=="subtitles"&&a.addTrackButton(a.tracks[f].srclang,a.tracks[f].isTranslation);a.loadNextTrack();e.addEventListener("timeupdate",function(){a.displayCaptions()},false);e.addEventListener("loadedmetadata",function(){a.displayChapters()},false);a.container.hover(function(){a.hasChapters&&(a.chapters.css("visibility","visible"),a.chapters.fadeIn(200))},function(){a.hasChapters&&!e.paused&&a.chapters.fadeOut(200,function(){b(this).css("visibility",
...
""&&(a.tracks.push({srclang:lang,src:null,entries:[],isLoaded:false,isTranslation:true}),!a.isLoadingTrack))a.trackToLoad--,a.addTrackButton(lang,true),a.options.startLanguage=lang,a.loadNextTrack()})}}},loadNextTrack:function(){this.trackToLoad++;this.trackToLoad<this.tracks.length?(this.isLoadingTrack=true,this.loadTrack(this.trackToLoad)):this.isLoadingTrack=false},loadTrack:function(a){var d=this,c=d.tracks[a],e=function(){c.isLoaded=true;d.enableTrackButton(c.srclang);d.loadNextTrack()};c.isTranslation?
mejs.TrackFormatParser.translateTrackText(d.tracks[0].entries,d.tracks[0].srclang,c.srclang,d.options.googleApiKey,function(a){c.entries=a;e()}):b.ajax({url:c.src,success:function(a){c.entries=mejs.TrackFormatParser.parse(a);e();c.kind=="chapters"&&d.media.duration>0&&d.drawChapters(c)},error:function(){d.loadNextTrack()}})},enableTrackButton:function(a){this.captionsButton.find("input[value="+a+"]").prop("disabled",false).siblings("label").html(mejs.language.codes[a]||a);this.options.startLanguage==
