...
class JElementZooFeedGlobal extends JElement {
	...
	function fetchElement($name, $value, &$node, $control_name) {
		...
		$html[] = $app->html->_('select.booleanlist', ($global ? $id : $control_name.'['.$name.']'), array('data-control' => $control_name.'['.$name.']') , $value);
		...
		$html[] = $app->html->_('control.text', ($global ? $id : $control_name.'[feed_title]'), $feed_title, array('id' => $id.'-feed_title', 'class' => 'feed-input-control', 'data-control' => $control_name.'[feed_title]'));
		$html[] = '</div>';
		$html[] = '<div class="feed-input">';
		$html[] = '<label class="hasTip" title="'.JText::_('ALTERNATE_FEED_LINK').'" for="'.$id.'-alternate_feed_link">'.JText::_('Alternate feed link').'</label>';
		$html[] = $app->html->_('control.text', ($global ? $id : $control_name.'[alternate_feed_link]'), $alternate_feed_link, array('id' => $id.'-alternate_feed_link', 'class' => 'feed-input-control', 'data-control' => $control_name.'[alternate_feed_link]'));
		$html[] = '</div>';
		$html[] = '</div>';
