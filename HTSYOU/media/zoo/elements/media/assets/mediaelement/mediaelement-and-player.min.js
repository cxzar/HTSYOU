...
mejs.Utility={encodeUrl:function(b){return encodeURIComponent(b)},escapeHTML:function(b){return b.toString().split("&").join("&amp;").split("<").join("&lt;").split('"').join("&quot;")},absolutizeUrl:function(b){var a=document.createElement("div");a.innerHTML='<a href="'+this.escapeHTML(b)+'">x</a>';return a.firstChild.href},getScriptPath:function(b){for(var a=0,c,d="",e="",f,g=document.getElementsByTagName("script");a<g.length;a++){f=g[a].src;for(c=0;c<b.length;c++)if(e=b[c],f.indexOf(e)>-1){d=f.substring(0,
f.indexOf(e));break}if(d!=="")break}return d},secondsToTimeCode:function(b,a,c,d){typeof c=="undefined"?c=false:typeof d=="undefined"&&(d=25);var e=Math.floor(b/3600)%24,f=Math.floor(b/60)%60,g=Math.floor(b%60),b=Math.floor((b%1*d).toFixed(3));return(a||e>0?(e<10?"0"+e:e)+":":"")+(f<10?"0"+f:f)+":"+(g<10?"0"+g:g)+(c?":"+(b<10?"0"+b:b):"")},timeCodeToSeconds:function(b,a,c,d){typeof c=="undefined"?c=false:typeof d=="undefined"&&(d=25);var b=b.split(":"),a=parseInt(b[0],10),e=parseInt(b[1],10),f=parseInt(b[2],
10),g=0;c&&(g=parseInt(b[3])/d);return a*3600+e*60+f+g},removeSwf:function(b){var a=document.getElementById(b);if(a&&a.nodeName=="OBJECT")mejs.MediaFeatures.isIE?(a.style.display="none",function(){a.readyState==4?mejs.Utility.removeObjectInIE(b):setTimeout(arguments.callee,10)}()):a.parentNode.removeChild(a)},removeObjectInIE:function(b){if(b=document.getElementById(b)){for(var a in b)typeof b[a]=="function"&&(b[a]=null);b.parentNode.removeChild(b)}}};
...
c)}},remove:function(){mejs.Utility.removeSwf(this.pluginElement.id)}};
...
a=parseInt(d*c/a,10);this.container.parent()[0].tagName.toLowerCase()==="body"&&(d=b(window).width(),a=b(window).height());this.container.width(d).height(a);this.$media.width("100%").height("100%");this.container.find("object, embed, iframe").width("100%").height("100%");this.media.setVideoSize&&this.media.setVideoSize(d,a);this.layers.children(".mejs-layer").width("100%").height("100%")}else this.container.width(this.width).height(this.height),this.layers.children(".mejs-layer").width(this.width).height(this.height)},
setControlsSize:function(){var a=0,c=0,d=this.controls.find(".mejs-time-rail"),e=this.controls.find(".mejs-time-total");this.controls.find(".mejs-time-current");this.controls.find(".mejs-time-loaded");others=d.siblings();this.options&&!this.options.autosizeProgress&&(c=parseInt(d.css("width")));if(c===0||!c)others.each(function(){b(this).css("position")!="absolute"&&(a+=b(this).outerWidth(true))}),c=this.controls.width()-a-(d.outerWidth(true)-d.outerWidth(false));d.width(c);e.width(c-(e.outerWidth(true)-
e.width()));this.setProgressRail&&this.setProgressRail();this.setCurrentRail&&this.setCurrentRail()},buildposter:function(a,c,d,e){var f=b('<div class="mejs-poster mejs-layer"></div>').appendTo(d),c=a.$media.attr("poster");if(a.options.poster!=="")c=a.options.poster;c!==""&&c!=null?this.setPoster(c):f.hide();e.addEventListener("play",function(){f.hide()},false)},setPoster:function(a){var c=this.container.find(".mejs-poster"),d=c.find("img");d.length==0&&(d=b('<img width="100%" height="100%" />').appendTo(c));
d.attr("src",a)},buildoverlays:function(a,c,d,e){if(a.isVideo){var f=b('<div class="mejs-overlay mejs-layer"><div class="mejs-overlay-loading"><span></span></div></div>').hide().appendTo(d),g=b('<div class="mejs-overlay mejs-layer"><div class="mejs-overlay-error"></div></div>').hide().appendTo(d),j=b('<div class="mejs-overlay mejs-layer mejs-overlay-play"><div class="mejs-overlay-button"></div></div>').appendTo(d).click(function(){e.paused?e.play():e.pause()});e.addEventListener("play",function(){j.hide();
f.hide();g.hide()},false);e.addEventListener("playing",function(){j.hide();f.hide();g.hide()},false);e.addEventListener("pause",function(){mejs.MediaFeatures.isiPhone||j.show()},false);e.addEventListener("waiting",function(){f.show()},false);e.addEventListener("loadeddata",function(){f.show()},false);e.addEventListener("canplay",function(){f.hide()},false);e.addEventListener("error",function(){f.hide();g.show();g.find("mejs-overlay-error").html("Error loading this resource")},false)}},buildkeyboard:function(a,
...
src:e.attr("src"),kind:e.attr("kind"),label:e.attr("label")||"",entries:[],isLoaded:false})})},changeSkin:function(a){this.container[0].className="mejs-container "+a;this.setPlayerSize();this.setControlsSize()},play:function(){this.media.play()},pause:function(){this.media.pause()},load:function(){this.media.load()},setMuted:function(a){this.media.setMuted(a)},setCurrentTime:function(a){this.media.setCurrentTime(a)},getCurrentTime:function(){return this.media.currentTime},setVolume:function(a){this.media.setVolume(a)},
getVolume:function(){return this.media.volume},setSrc:function(a){this.media.setSrc(a)},remove:function(){this.media.pluginType=="flash"?this.media.remove():this.media.pluginTyp=="native"&&this.media.prop("controls",true);this.isDynamic||this.$node.insertBefore(this.container);this.container.remove()}};if(typeof jQuery!="undefined")jQuery.fn.mediaelementplayer=function(a){return this.each(function(){new mejs.MediaElementPlayer(this,a)})};b(document).ready(function(){b(".mejs-player").mediaelementplayer()});
...
(function(b){b.extend(MediaElementPlayer.prototype,{buildprogress:function(a,c,d,e){b('<div class="mejs-time-rail"><span class="mejs-time-total"><span class="mejs-time-loaded"></span><span class="mejs-time-current"></span><span class="mejs-time-handle"></span><span class="mejs-time-float"><span class="mejs-time-float-current">00:00</span><span class="mejs-time-float-corner"></span></span></span></div>').appendTo(c);var f=c.find(".mejs-time-total"),d=c.find(".mejs-time-loaded"),g=c.find(".mejs-time-current"),
j=c.find(".mejs-time-handle"),h=c.find(".mejs-time-float"),l=c.find(".mejs-time-float-current"),k=function(a){var a=a.pageX,b=f.offset(),c=f.outerWidth(),d=0,g=a-b.left;a>b.left&&a<=c+b.left&&e.duration&&(d=(a-b.left)/c,d=d<=0.02?0:d*e.duration,m&&e.setCurrentTime(d),mejs.MediaFeatures.hasTouch||(h.css("left",g),l.html(mejs.Utility.secondsToTimeCode(d)),h.show()))},m=false,n=false;f.bind("mousedown",function(a){if(a.which===1)return m=true,k(a),false});c.find(".mejs-time-total").bind("mouseenter",
function(){n=true;mejs.MediaFeatures.hasTouch||h.show()}).bind("mouseleave",function(){n=false;h.hide()});b(document).bind("mouseup",function(){m=false;h.hide()}).bind("mousemove",function(a){(m||n)&&k(a)});e.addEventListener("progress",function(b){a.setProgressRail(b);a.setCurrentRail(b)},false);e.addEventListener("timeupdate",function(b){a.setProgressRail(b);a.setCurrentRail(b)},false);this.loaded=d;this.total=f;this.current=g;this.handle=j},setProgressRail:function(a){var b=a!=void 0?a.target:
this.media,d=null;b&&b.buffered&&b.buffered.length>0&&b.buffered.end&&b.duration?d=b.buffered.end(0)/b.duration:b&&b.bytesTotal!=void 0&&b.bytesTotal>0&&b.bufferedBytes!=void 0?d=b.bufferedBytes/b.bytesTotal:a&&a.lengthComputable&&a.total!=0&&(d=a.loaded/a.total);d!==null&&(d=Math.min(1,Math.max(0,d)),this.loaded&&this.total&&this.loaded.width(this.total.width()*d))},setCurrentRail:function(){if(this.media.currentTime!=void 0&&this.media.duration&&this.total&&this.handle){var a=this.total.width()*
...
(function(b){b.extend(mejs.MepDefaults,{muteText:"Mute Toggle",hideVolumeOnTouchDevices:true});b.extend(MediaElementPlayer.prototype,{buildvolume:function(a,c,d,e){if(!mejs.MediaFeatures.hasTouch||!this.options.hideVolumeOnTouchDevices){var f=b('<div class="mejs-button mejs-volume-button mejs-mute"><button type="button" aria-controls="'+this.id+'" title="'+this.options.muteText+'"></button><div class="mejs-volume-slider"><div class="mejs-volume-total"></div><div class="mejs-volume-current"></div><div class="mejs-volume-handle"></div></div></div>').appendTo(c),
g=f.find(".mejs-volume-slider"),j=f.find(".mejs-volume-total"),h=f.find(".mejs-volume-current"),l=f.find(".mejs-volume-handle"),k=function(a){if(g.is(":visible")){var b=j.height(),c=j.position(),a=b-b*a;l.css("top",c.top+a-l.height()/2);h.height(b-a);h.css("top",c.top+a)}else g.show(),k(a),g.hide()},m=function(a){var b=j.height(),c=j.offset(),d=parseInt(j.css("top").replace(/px/,""),10),a=a.pageY-c.top;c.top==0||c.left==0||(c=Math.max(0,(b-a)/b),c=Math.min(c,1),a<0?a=0:a>b&&(a=b),l.css("top",a-l.height()/
2+d),h.height(b-a),h.css("top",a+d),c==0?(e.setMuted(true),f.removeClass("mejs-mute").addClass("mejs-unmute")):(e.setMuted(false),f.removeClass("mejs-unmute").addClass("mejs-mute")),c=Math.max(0,c),c=Math.min(c,1),e.setVolume(c))},n=false,o=false;f.hover(function(){g.show();o=true},function(){o=false;n||g.hide()});g.bind("mouseover",function(){o=true}).bind("mousedown",function(a){m(a);n=true;return false});b(document).bind("mouseup",function(){n=false;o||g.hide()}).bind("mousemove",function(a){n&&
...
(function(b){b.extend(mejs.MepDefaults,{usePluginFullScreen:true,newWindowCallback:function(){return""},fullscreenText:"Fullscreen"});b.extend(MediaElementPlayer.prototype,{isFullScreen:false,isNativeFullScreen:false,docStyleOverflow:null,isInIframe:false,buildfullscreen:function(a,c,d,e){if(a.isVideo){a.isInIframe=window.location!=window.parent.location;mejs.MediaFeatures.hasTrueNativeFullScreen&&a.container.bind(mejs.MediaFeatures.fullScreenEventName,function(){mejs.MediaFeatures.isFullScreen()?
(a.isNativeFullScreen=true,a.setControlsSize()):(a.isNativeFullScreen=false,a.exitFullScreen())});var f=this,g=b('<div class="mejs-button mejs-fullscreen-button"><button type="button" aria-controls="'+f.id+'" title="'+f.options.fullscreenText+'"></button></div>').appendTo(c);if(f.media.pluginType==="native"||!f.options.usePluginFullScreen&&!mejs.MediaFeatures.isFirefox)g.click(function(){mejs.MediaFeatures.hasTrueNativeFullScreen&&mejs.MediaFeatures.isFullScreen()||a.isFullScreen?a.exitFullScreen():
...
g.mouseover(function(){if(!f.isFullScreen){var b=g.offset(),c=a.container.offset();e.positionFullscreenButton(b.left-c.left,b.top-c.top,false);g.css("pointer-events","none");f.controls.css("pointer-events","none");k.show();n.show();m.show();o();h=true}});e.addEventListener("fullscreenchange",function(){l()})}else g.mouseover(function(){j!==null&&(clearTimeout(j),delete j);var b=g.offset(),c=a.container.offset();e.positionFullscreenButton(b.left-c.left,b.top-c.top,true)}).mouseout(function(){j!==null&&
...
this.captions.show();return}this.captions.hide()}},displayChapters:function(){var a;for(a=0;a<this.tracks.length;a++)if(this.tracks[a].kind=="chapters"&&this.tracks[a].isLoaded){this.drawChapters(this.tracks[a]);this.hasChapters=true;break}},drawChapters:function(a){var c=this,d,e,f=e=0;c.chapters.empty();for(d=0;d<a.entries.times.length;d++){e=a.entries.times[d].stop-a.entries.times[d].start;e=Math.floor(e/c.media.duration*100);if(e+f>100||d==a.entries.times.length-1&&e+f<100)e=100-f;c.chapters.append(b('<div class="mejs-chapter" rel="'+
