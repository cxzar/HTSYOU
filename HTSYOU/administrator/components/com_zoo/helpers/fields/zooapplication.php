...
foreach ($table->all(array('order' => 'name')) as $application) {
	...
	if ($node->attributes('categories') || @$node->attributes()->categories) {
		...
		$opts    = $node->attributes('frontpage') || @$node->attributes()->frontpage ? array($this->app->html->_('select.option', '', '&#8226;	'.JText::_('Frontpage'))) : array();
	...
	if ($node->attributes('types') || @$node->attributes()->types) {
		...
		$types[] = $this->app->html->_('select.genericlist', $opts, $control_name.'[type]', $attribs, 'value', 'text', $params->get('type'));
	}
}
